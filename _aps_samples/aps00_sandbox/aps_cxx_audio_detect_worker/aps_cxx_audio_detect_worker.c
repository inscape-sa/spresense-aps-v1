...
int main(void)
{
  ...
  ret = mpmutex_init(&mutex, APS_CXX_AUDIO_DETECTKEY_MUTEX);
  ASSERT(ret == 0);
  ...
  ret = mpmq_init(&mq, APS_CXX_AUDIO_DETECTKEY_MQ, 0);
  ASSERT(ret == 0);

  /* Initialize MP shared memory */

  ret = mpshm_init(&shm, APS_CXX_AUDIO_DETECTKEY_SHM, 1024);
  ASSERT(ret == 0);
  ...
  ASSERT(ret == MSG_ID_APS_CXX_AUDIO_DETECT);
  ...
  ret = mpmq_send(&mq, MSG_ID_APS_CXX_AUDIO_DETECT, msgdata);
  ASSERT(ret == 0);
