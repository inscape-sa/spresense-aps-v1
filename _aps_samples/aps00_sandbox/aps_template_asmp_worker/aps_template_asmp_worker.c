...
int main(void)
{
  ...
  ret = mpmutex_init(&mutex, APS_TEMPLATE_ASMPKEY_MUTEX);
  ASSERT(ret == 0);
  ...
  ret = mpmq_init(&mq, APS_TEMPLATE_ASMPKEY_MQ, 0);
  ASSERT(ret == 0);

  /* Initialize MP shared memory */

  ret = mpshm_init(&shm, APS_TEMPLATE_ASMPKEY_SHM, 1024);
  ASSERT(ret == 0);
  ...
  ASSERT(ret == MSG_ID_APS_TEMPLATE_ASMP);
  ...
  ret = mpmq_send(&mq, MSG_ID_APS_TEMPLATE_ASMP, msgdata);
  ASSERT(ret == 0);
