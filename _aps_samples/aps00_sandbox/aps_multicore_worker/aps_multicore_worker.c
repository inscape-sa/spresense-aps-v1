...
int main(void)
{
  ...
  ret = mpmutex_init(&mutex, APS00_SANDBOX_APS_MULTICOREKEY_MUTEX);
  ASSERT(ret == 0);
  ...
  ret = mpmq_init(&mq, APS00_SANDBOX_APS_MULTICOREKEY_MQ, 0);
  ASSERT(ret == 0);

  /* Initialize MP shared memory */

  ret = mpshm_init(&shm, APS00_SANDBOX_APS_MULTICOREKEY_SHM, 1024);
  ASSERT(ret == 0);
  ...
  ASSERT(ret == MSG_ID_APS00_SANDBOX_APS_MULTICORE);
  ...
  ret = mpmq_send(&mq, MSG_ID_APS00_SANDBOX_APS_MULTICORE, msgdata);
  ASSERT(ret == 0);
