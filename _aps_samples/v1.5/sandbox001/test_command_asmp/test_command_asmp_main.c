...
/* Worker ELF path */
#define WORKER_FILE "/mnt/spif/test_asmp"
...
int test_command_asmp_main(int argc, char *argv[])
{
  mptask_t mptask;
  mpmutex_t mutex;
  mpshm_t shm;
  ...
  ret = mpmutex_init(&mutex, TEST_ASMPKEY_MUTEX);
  if (ret < 0)
    {
  ...
  ret = mpmq_init(&mq, TEST_ASMPKEY_MQ, mptask_getcpuid(&mptask));
  if (ret < 0)
    {
  ...
  ret = mpshm_init(&shm, TEST_ASMPKEY_SHM, 1024);
  if (ret < 0)
    {
  ...
  ret = mpmq_send(&mq, MSG_ID_TEST_ASMP, (uint32_t) &data);
  if (ret < 0)
    {
