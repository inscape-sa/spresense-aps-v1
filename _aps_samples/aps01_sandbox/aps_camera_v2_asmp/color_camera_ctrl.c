...
#define STATIC_ALLOC_SIZE   (IMAGE_YUV_SIZE)
#define ALIGNED_ADDR (32)
uint8_t _buflist1[STATIC_ALLOC_SIZE] __attribute__((aligned(ALIGNED_ADDR)));
uint8_t _buflist2[STATIC_ALLOC_SIZE] __attribute__((aligned(ALIGNED_ADDR)));
...
#if 1
          if (cnt == 0) {
            bufferlist[cnt] = _buflist1;
          } else if (cnt == 1) {
            bufferlist[cnt] = _buflist2;
          } else {
            /* ERROR */
            printf("Out of memory at alloc_buffer[%d] (ERRNO=%d)\n", cnt, errno);
            return;
          }
          printf("Setbuf 0x%x at alloc_buffer[%d]\n", bufferlist[cnt], cnt);
#else
